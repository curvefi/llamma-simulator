...
class Simulator:
    ...
    def __init__(self, filename, ext_fee, add_reverse=False,
        ...
        self.ema_filename = filename.replace('.json', '-ema.json')
    ...
    def load_prices(self):
        ...
        _price_data = {self.filename: [], self.ema_filename: []}

        for fname in [self.filename, self.ema_filename]:
            ...
            _price_data[fname] = data

        price_data = _price_data[self.filename]
        ema_data = _price_data[self.ema_filename]
    ...
    def update_emas(self, Texp):
        if self.ema_time != Texp:
            ...
            ema = ema_data[0][1]
            ema_t = ema_data[0][0]
            for t, _, high, low, _, _ in ema_data:
